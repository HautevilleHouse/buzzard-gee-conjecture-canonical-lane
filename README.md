...
Canonical Lane research workspace for a specialist problem in automorphic forms and Galois representations:
proving persistence of the expected automorphic-Galois compatibility package through an admissible compatibility closure architecture.

## Main Manuscript

- [paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md](paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md)
...
- `paper/`
  - `BUZZARD_GEE_CONJECTURE_PREPRINT.md`
...
1. Theorem chain first: read `paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md`.
2. Constants provenance second: audit `paper/CANONICAL_ROUTING_INDEX.md`, `artifacts/constants_extraction_inputs.json`, `artifacts/constants_extracted.json`, and `artifacts/promotion_report.json`.
3. Pipeline third: audit `paper/EXTRACTION_SPEC.md`, `artifacts/constants_extraction_inputs.json`, `artifacts/constants_extracted.json`, and `artifacts/promotion_report.json`.
...
Current BZG runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/bzg_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/bzg_closure_guard.py`.
...
- Manuscript target: [paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md](paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md)
