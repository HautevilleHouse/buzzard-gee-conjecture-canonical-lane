...
In-paper anchor: `paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md` (`BZG_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the expected automorphic-Galois compatibility package through an admissible compatibility closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`BZG_G3` closes when `kappa_compact` provides the compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_BZG`.
...
If near-failure extraction and no-Zeno spacing hold, then `BZG_G3` prevents hidden drift before rigidity is applied.
