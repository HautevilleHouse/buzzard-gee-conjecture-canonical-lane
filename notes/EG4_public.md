...
In-paper anchor: `paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md` (`BZG_G4` and `BZG_G5`).

## Goal
Separate the rigidity job from the endpoint-transfer job for `proving persistence of the expected automorphic-Galois compatibility package through an admissible compatibility closure architecture`.
...
- endpoint-transfer lock: `automorphic_lock`.
- coherence interface: `eps_coh` remains available to the bridge and final margin.

## Closure Criterion

`BZG_G4` closes when `rho_rigidity` excludes bad endpoint alternatives: bad countermodels are excluded.
`BZG_G5` closes when `automorphic_lock` transfers the surviving endpoint to the intended target class: rigid limit transfers to the intended endpoint class.
Together they feed the strict margin `M_BZG`.
...
The surviving rigid representative is locked to the standard problem-side endpoint by `automorphic_lock`.

Payload: read this note together with `notes/IDENTIFICATION_BRIDGE.md`.

### Theorem EG4.3 (rigidity/transfer gate closure)
If bad limits are excluded and the endpoint lock is active, then `BZG_G4` and `BZG_G5` deliver the boundary object needed by the final margin.

## Current Instantiation

- rigidity gate: `BZG_G4`
- rigidity artifact key: `rho_rigidity`
- transfer gate: `BZG_G5`
- transfer artifact key: `automorphic_lock`
