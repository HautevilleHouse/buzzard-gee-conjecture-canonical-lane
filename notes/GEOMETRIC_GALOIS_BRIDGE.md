...
In-paper anchor: `paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md` (geometric/arithmetic bridge layer).
...
- transfer lock: `automorphic_lock`.
- coherence remainder: `eps_coh`.

## Closure Criterion

The bridge is valid only when the admissible visible sector is projected, the remainder is named, and the endpoint-transfer lock `automorphic_lock` connects the canonical endpoint to the ordinary problem-side formulation.
