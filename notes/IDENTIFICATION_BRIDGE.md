...
In-paper anchor: `paper/BUZZARD_GEE_CONJECTURE_PREPRINT.md` (bridge and final strict margin).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of the expected automorphic-Galois compatibility package through an admissible compatibility closure architecture`.
...
2. the endpoint-transfer lock `automorphic_lock` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_BZG` remains positive.
...
Payload: verify `automorphic_lock` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_BZG` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `automorphic_lock`
